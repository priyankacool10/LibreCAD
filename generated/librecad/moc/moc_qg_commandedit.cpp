/****************************************************************************
** Meta object code from reading C++ file 'qg_commandedit.h'
**
** Created: Tue Jun 18 04:10:07 2013
**      by: The Qt Meta Object Compiler version 63 (Qt 4.8.1)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../../../librecad/src/ui/qg_commandedit.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'qg_commandedit.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 63
#error "This file was generated using the moc from 4.8.1. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_QG_CommandEdit[] = {

 // content:
       6,       // revision
       0,       // classname
       0,    0, // classinfo
       5,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       5,       // signalCount

 // signals: signature, parameters, type, tag, flags
      16,   15,   15,   15, 0x05,
      29,   15,   15,   15, 0x05,
      38,   15,   15,   15, 0x05,
      48,   15,   15,   15, 0x05,
      59,   15,   15,   15, 0x05,

       0        // eod
};

static const char qt_meta_stringdata_QG_CommandEdit[] = {
    "QG_CommandEdit\0\0tabPressed()\0escape()\0"
    "focusIn()\0focusOut()\0clearCommandsHistory()\0"
};

void QG_CommandEdit::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        Q_ASSERT(staticMetaObject.cast(_o));
        QG_CommandEdit *_t = static_cast<QG_CommandEdit *>(_o);
        switch (_id) {
        case 0: _t->tabPressed(); break;
        case 1: _t->escape(); break;
        case 2: _t->focusIn(); break;
        case 3: _t->focusOut(); break;
        case 4: _t->clearCommandsHistory(); break;
        default: ;
        }
    }
    Q_UNUSED(_a);
}

const QMetaObjectExtraData QG_CommandEdit::staticMetaObjectExtraData = {
    0,  qt_static_metacall 
};

const QMetaObject QG_CommandEdit::staticMetaObject = {
    { &QLineEdit::staticMetaObject, qt_meta_stringdata_QG_CommandEdit,
      qt_meta_data_QG_CommandEdit, &staticMetaObjectExtraData }
};

#ifdef Q_NO_DATA_RELOCATION
const QMetaObject &QG_CommandEdit::getStaticMetaObject() { return staticMetaObject; }
#endif //Q_NO_DATA_RELOCATION

const QMetaObject *QG_CommandEdit::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->metaObject : &staticMetaObject;
}

void *QG_CommandEdit::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_QG_CommandEdit))
        return static_cast<void*>(const_cast< QG_CommandEdit*>(this));
    return QLineEdit::qt_metacast(_clname);
}

int QG_CommandEdit::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QLineEdit::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 5)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 5;
    }
    return _id;
}

// SIGNAL 0
void QG_CommandEdit::tabPressed()
{
    QMetaObject::activate(this, &staticMetaObject, 0, 0);
}

// SIGNAL 1
void QG_CommandEdit::escape()
{
    QMetaObject::activate(this, &staticMetaObject, 1, 0);
}

// SIGNAL 2
void QG_CommandEdit::focusIn()
{
    QMetaObject::activate(this, &staticMetaObject, 2, 0);
}

// SIGNAL 3
void QG_CommandEdit::focusOut()
{
    QMetaObject::activate(this, &staticMetaObject, 3, 0);
}

// SIGNAL 4
void QG_CommandEdit::clearCommandsHistory()
{
    QMetaObject::activate(this, &staticMetaObject, 4, 0);
}
QT_END_MOC_NAMESPACE
